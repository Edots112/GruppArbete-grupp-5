import Time from "./time.js";
 ...
 class Sheet {
    ...
    createGenesisBlock() {
        let genesisBlock =  new Time({"user": "Genesis", "work": 0, "isPoll": "not a poll" });
    ...
    async isChainValid() {
        ...
        for (let i = 1; i < this.timeSheet.length; i++) {
            ...
            if (currentBlock.hash !== currentBlockHash) {
              console.log("INVALID! Not same hash!", currentBlock.hash, currentBlockHash);
              return false;
            }
    ...
    findBlocksByUserId(userId) {
        ...
        for (const block of blocks) {
          ...
          blockElement.innerHTML = "<p>"+"PREVIOUS HASH: "+block.prevHash+"<br/>"+block.data.user+"<br/>"+block.data.work+ block.data.isPoll+"<br/>"+"THIS-HASH: " +"<br/>" +block.hash+" <br/> "+"UserId: " + block.userId+"</p>";
      ...
      findPollBlocks() {
        ...
        for (const pollBlock of pollBlocks) {
          ...
          blockElement.className = "blockElement";
          blockElement.style = "overflow: scroll; padding: 10px; margin: 10px;";
          blockElement.innerHTML = "<p>"+"PREVIOUS HASH: "+pollBlock.prevHash+"<br/>"+pollBlock.data.user+"<br/>"+pollBlock.data.work+ pollBlock.data.isPoll+"<br/>"+"THIS-HASH: " +"<br/>" +pollBlock.hash+" <br/> "+"UserId: " + pollBlock.userId+"</p>";
